...
mod tests {
    ...
    fn tokenize() {
        ...
        let mut lexer = Lexer::new("some_identifier123");
        let tokens = lexer.tokenize().unwrap();
        assert_eq!(tokens[0].token_type, TokenType::Identifier(String::from("some_identifier123")));
        assert_eq!(tokens.len(), 1);

        let mut lexer = Lexer::new("9");
        let tokens = lexer.tokenize().unwrap();
        assert_eq!(tokens.len(), 1);
        assert_ne!(tokens[0].token_type, TokenType::Integer(5));

        let mut lexer = Lexer::new("9 + 5");
        let tokens = lexer.tokenize().unwrap();
        assert_eq!(tokens.len(), 3);
        assert_eq!(tokens[0].token_type, TokenType::Integer(9));
        ...
        assert_eq!(tokens[2].token_type, TokenType::Integer(6));
        ...
        assert_eq!(tokens[1].token_type, TokenType::Integer(20));
        ...
        assert_eq!(tokens[7].token_type, TokenType::Integer(2));
        ...
        let mut lexer = Lexer::new("9. + .5 * 1.2");
        let tokens = lexer.tokenize().unwrap();
        // assert_eq!(tokens.len(), 5);
        assert_eq!(tokens[0].token_type, TokenType::Float(9.0));
        assert_eq!(tokens[1].token_type, TokenType::Plus);
        assert_eq!(tokens[2].token_type, TokenType::Float(0.5));
        assert_eq!(tokens[3].token_type, TokenType::Asterisk);
        assert_eq!(tokens[4].token_type, TokenType::Float(1.2));
    }
}
